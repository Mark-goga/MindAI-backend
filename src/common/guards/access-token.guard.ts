...
export class AccessTokenGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext) {
    ...
    if (!authorization) {
      throw new UnauthorizedException(ERROR_MESSAGES.AUTH.AUTHORIZATION_NOT_FOUND);
    }

    const [scheme, token] = authorization.split(' ');
    if (scheme !== 'Bearer' || !token) {
      throw new UnauthorizedException(ERROR_MESSAGES.AUTH.AUTHORIZATION_INVALID);
    }
    ...
    if (!session || session.userId !== user.id || session.applicationId !== payload.applicationId) {
